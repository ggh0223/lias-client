          ...
          <div className="bg-white rounded-lg p-4 border border-blue-200">
            ...
            currentVersion.approvalLineInfo.steps.length > 0 ? (
              <div className="space-y-2">
                ...
                {currentVersion.approvalLineInfo.steps.map((step, index) => (
                  <div
                    key={step.id}
                    className="bg-gray-50 rounded-md p-3 border border-gray-200"
                  >
                    <div className="flex items-start space-x-3">
                      <div className="flex-shrink-0 w-6 h-6 rounded-full bg-blue-100 text-blue-800 flex items-center justify-center text-xs font-semibold">
                        {index + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        ...
                        {step.defaultApproverId && (
                          <p className="text-xs text-gray-600">
                            기본 담당자: {step.defaultApproverId}
                          </p>
                        )}
                        {step.targetDepartmentId && (
                          <p className="text-xs text-gray-600">
                            대상 부서: {step.targetDepartmentId}
                          </p>
                        )}
                        {step.targetPositionId && (
                          <p className="text-xs text-gray-600">
                            대상 직책: {step.targetPositionId}
                          </p>
                        )}
                      </div>
                    </div>
